...
internal sealed class GetInvoiceQueryHandler(IUnitOfWork unitOfWork, IMapper mapper)
    : IQueryHandler<GetInvoiceQuery, InvoiceResponse>
{
    ...
    public async Task<Result<InvoiceResponse>> Handle(GetInvoiceQuery request, CancellationToken cancellationToken)
    {
        ...
        if (invoice is null)
            return Result<InvoiceResponse>.Failure(HttpStatusCode.BadRequest, "Null.Error",
